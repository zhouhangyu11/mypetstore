...
public class AddItemToCartServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(cart.containsItemId(workingItemId)){
            ...
            cartItem.itemId=workingItemId;

            //明天记得改product Id
            cartItem.productId=item.getProduct().getProductId();

            cartItem.description = item.getAttribute1();
            cartItem.instock = isInStock?1:0;

            cartItem.quantity = 1;

            cartItem.price = item.getListPrice().floatValue();

            cartItem.total1 = item.getListPrice().floatValue();
